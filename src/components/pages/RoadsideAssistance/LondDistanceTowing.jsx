...
import { 
  FaRoad, FaCheckCircle, FaArrowRight, FaPhoneAlt,
  FaHeadset, FaShieldAlt, FaUsers, FaClock,
  FaMapMarkerAlt, FaTruck, FaCar, FaCompass, FaGlobe
} from 'react-icons/fa';

const LongDistanceTowing = () => {
  const features = [
    { icon: <FaRoad className="text-xl md:text-2xl" />, title: 'Long Distance', desc: 'Inter-city towing' },
  ...
  const routes = [
    { name: 'Inter-City', icon: <FaMapMarkerAlt className="text-3xl md:text-4xl" /> },
    { name: 'Inter-State', icon: <FaRoad className="text-3xl md:text-4xl" /> },
    { name: 'Nationwide', icon: <FaGlobe className="text-3xl md:text-4xl" /> },
    { name: 'Cross-Country', icon: <FaCompass className="text-3xl md:text-4xl" /> },
  ];
          ...
          <div className="max-w-3xl">
            ...
            >
              <span className="inline-flex items-center gap-2 bg-sky-500/20 backdrop-blur-md border border-sky-400/30 rounded-full px-3 md:px-4 py-1.5 md:py-2 mb-4 md:mb-6">
                <FaRoad className="text-sky-300 text-sm md:text-base" />
                <span className="text-white text-xs md:text-sm font-medium">Long Distance Towing</span>
              </span>
              ...
              <p className="text-white/90 text-sm sm:text-base md:text-lg mt-3 md:mt-4 max-w-2xl">
                Reliable long distance towing for all vehicle types. 
                Inter-city, inter-state, and nationwide coverage.
              </p>
        ...
        <div className="container-custom">
          <div className="text-center mb-8 md:mb-12">
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900">
              We Cover <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-500 to-sky-300">All Routes</span>
            </h2>
          </div>
            ...
            {routes.map((route, index) => (
              ...
              >
                <div className="w-16 h-16 bg-sky-50 rounded-full flex items-center justify-center mx-auto mb-2 md:mb-3 group-hover:bg-sky-100 transition-colors">
                  <span className="text-sky-500">{route.icon}</span>
                </div>
                <h4 className="font-semibold text-gray-900 text-sm md:text-base">{route.name}</h4>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="relative py-10 md:py-14 overflow-hidden">
        ...
        <div className="container-custom relative z-10">
          <div className="max-w-3xl mx-auto text-center text-white">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold">Need Long Distance Towing?</h2>
            <p className="text-white/80 mt-2 md:mt-3 text-sm sm:text-base">
              Reliable towing for all your long distance needs.
            </p>
